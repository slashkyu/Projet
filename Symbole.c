...
Elf32_Shdr *find_section(Elf32_Ehdr* data, char *string, int type){
        Elf32_Shdr *section_header_start = (Elf32_Shdr*)((void*)data + data->e_s                                                                                                                                                                                               hoff);
        Elf32_Shdr *section_header;int i;
        //On r̩cup̬re la section string
        Elf32_Shdr section_string = section_header_start[data->e_shstrndx];
        //On r̩cup̬re la premi̬re string
        char *strings = (char*)((void*)data + section_string.sh_offset);


        for ( i = 0; i < data->e_shnum; i++){
                section_header = section_header_start+i;
                if((section_header->sh_type == type) && (strcmp(strings + sectio                                                                                                                                                                                               n_header->sh_name, string)==0) ){
...
void print_sym(Elf32_Ehdr * data)
{
                int i,nb;
        //Initialisation
        //On r̩cup̬re la section symbole
        Elf32_Shdr *symbole = find_section(data, ".symtab", 2);
        //On r̩cup̬re la section des noms des symboles
      ...
      if ((fd = open(argv[1], O_RDONLY)) > 0)
        {
          ...
          else if ((data = mmap(0, file_infos.st_size, PROT_READ, MAP_PRIVATE, f                                                                                                                                                                                               d, 0)) == MAP_FAILED)
