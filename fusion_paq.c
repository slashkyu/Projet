...
void fusion_section_table(Elf32_Ehdr * data1, Elf32_Ehdr *data2)
{
        ...
        Elf_1.table_section = section_header_start1;

        Elf_2.table_section = section_header_start2;

        Elf_final.table_section = malloc( fmax(data1->e_shnum,data2->e_shnum) );


        // on recupere un pointeur sur le debut de la section et donc sur la premiere chaine
        char *strings = (char*)((void*)data1 + sections_string.sh_offset);
        ...
        if (data1->e_shnum > data2->e_shnum) {
                for (i = 0; i < data1->e_shnum; i++){
                        ...
                        while (j < data2->e_shnum && !strcmp(strings + section_header_1->sh_name, strings + section_header_2->sh_name) && (section_header_1.sh_type == section_header_2.sh_type) ){
                        ...
                        if( j == data2->e_shnum) {
                                ...
                                //symtab ou dynsm
                                if(Elf_final.table_section[index]->sh_type == 2 || Elf_final.table_section[index] = 11) {
                                        Elf_final.table_section[index]->sh_link = data1->e_shnum;
                                        //insérer ici l'index + 1 du dernier symbole après l'étape 7

                                }
                                //Rela ou Rel
                                else if (Elf_final.table_section[index]->sh_type == 4 || Elf_final.table_section[index] = 9){
